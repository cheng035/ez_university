...
class ObjectUpdateMixin:
    ...
    def get(self, request,pk):
        currentModel=self.get_object(pk)
        context={
            'form':self.form_class(
    ...
    def post(self, request,pk):
        currentModel=self.get_object(pk)
        bound_form = self.form_class(request.POST,instance=currentModel)
        if bound_form.is_valid():
...
class ObjectDeleteMixin():
        ...
        def post(self, request, pk):
                model2 = self.get_object(pk)
                model2.delete()
                return redirect(self.template_name3)
