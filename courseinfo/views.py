...
from .forms import InstructorForm, SectionForm, CourseForm, SemesterForm, RegistrationForm, StudentForm
from .models import (Instructor,
                     Section,
                     Course, Semester, Registration, Student)
from .utils import ObjectCreateMixin, ObjectUpdateMixin, ObjectDeleteMixin, PageLinksMixin
...
class InstructorCreate(LoginRequiredMixin,PermissionRequiredMixin,CreateView):
    permission_required = 'courseinfo.add_instrucor'
    form_class = InstructorForm
    model=Instructor


class InstructorUpdate(LoginRequiredMixin,PermissionRequiredMixin,UpdateView):
    permission_required = 'courseinfo.change_instrucor'
    form_class = InstructorForm
    model = Instructor
    template_name = 'courseinfo/instructor_form_update.html'


# class InstructorDelete(View,ObjectDeleteMixin):
#     model=Instructor
#     related=Section
#     template_name='courseinfo/instructor_refuse_delete.html'
#     template_name2='courseinfo/instructor_confirm_delete.html'
#     template_name3='courseinfo_instructor_list_urlpattern'

class InstructorDelete(LoginRequiredMixin,PermissionRequiredMixin,View):
    permission_required = 'courseinfo.delete_instrucor'
...
class RegistrationDetail(LoginRequiredMixin,PermissionRequiredMixin,View):
    ...
    def get(self, request, pk):
        ...
        return render(
            request,
            'courseinfo/registratioin_detail.html',
            {
